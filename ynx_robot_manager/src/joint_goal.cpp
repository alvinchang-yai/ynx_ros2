...
namespace ynx_robot_manager
{
  ...
  rclcpp_action::GoalResponse YnxRobotManager::joint_goal_handle_goal(const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const JointGoal::Goal> goal) {
    (void)uuid;
  ...
  rclcpp_action::CancelResponse YnxRobotManager::joint_goal_handle_cancel(const std::shared_ptr<JointGoalHandle> goal_handle) {
  ...
  void YnxRobotManager::joint_goal_handle_accepted(const std::shared_ptr<JointGoalHandle> goal_handle) {
    std::thread{std::bind(&YnxRobotManager::joint_goal_handle_execution, this, std::placeholders::_1), goal_handle}.detach();
  }

  // --- Joint Goal - Handle Execution --- ///
  void YnxRobotManager::joint_goal_handle_execution(const std::shared_ptr<JointGoalHandle> goal_handle) {
  ...
  double YnxRobotManager::calculate_joint_distance(const std::vector<double>& current, const std::vector<double>& target) {
    double distance = 0.0;
