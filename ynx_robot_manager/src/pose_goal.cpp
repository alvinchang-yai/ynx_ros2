...
namespace ynx_robot_manager
{
  ...
  rclcpp_action::GoalResponse YnxRobotManager::pose_goal_handle_goal(const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const PoseGoal::Goal> goal) {
    (void)uuid;
  ...
  rclcpp_action::CancelResponse YnxRobotManager::pose_goal_handle_cancel(const std::shared_ptr<PoseGoalHandle> goal_handle) {
  ...
  void YnxRobotManager::pose_goal_handle_accepted(const std::shared_ptr<PoseGoalHandle> goal_handle) {
    std::thread{std::bind(&YnxRobotManager::pose_goal_handle_execution, this, std::placeholders::_1), goal_handle}.detach();
  }

  // --- Pose Goal - Handle Execution --- ///
  void YnxRobotManager::pose_goal_handle_execution(const std::shared_ptr<PoseGoalHandle> goal_handle) {
  ...
  double YnxRobotManager::calculate_cartesian_distance(const geometry_msgs::msg::Point& p1, const geometry_msgs::msg::Point& p2) {
    return std::sqrt(std::pow(p1.x - p2.x, 2) + std::pow(p1.y - p2.y, 2) + std::pow(p1.z - p2.z, 2));
  };
  // --- Helper Function - Check if transform is a child of tool0 --- ///
  bool YnxRobotManager::is_frame_tool0_child(const std::string& target_frame) {
